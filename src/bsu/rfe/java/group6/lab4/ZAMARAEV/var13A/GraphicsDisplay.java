package bsu.rfe.java.group6.lab4.ZAMARAEV.var13A;
...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[]{ 20,10, 5, 10, 5}, 0.0f);
...
// Отобразить график
    ...
    boolean squareOfDigits(double x) {
        int y = (int) x;
...
// Если сумма цифр в записи целой часьти значения функции в точке меньше десятси
            if (squareOfDigits(point[1]) == true) {
            ...
            canvas.draw(new Line2D.Double(shiftPoint(center, 11, 6), shiftPoint(center, 11, -6)));
            canvas.draw(new Line2D.Double(shiftPoint(center, 0, 11), shiftPoint(center, 0, -11)));
            canvas.draw(new Line2D.Double(shiftPoint(center, -11, 6), shiftPoint(center, -11, -6)));
            canvas.draw(new Line2D.Double(shiftPoint(center, -6, 11), shiftPoint(center, 6, 11)));
            canvas.draw(new Line2D.Double(shiftPoint(center, -6, -11), shiftPoint(center, 6, -11)));
...
// а нижняя (minY) <= 0.0
            canvas.draw(new Line2D.Double(xyToPoint(minX, 0.1),
                    xyToPoint(maxX, 0.1)));
// Стрелка оси X
            GeneralPath arrow = new GeneralPath();
// Установить начальную точку ломаной точно на правый конец оси X
            Point2D.Double lineEnd = xyToPoint(maxX, 0.1);
            arrow.moveTo(lineEnd.getX(), lineEnd.getY());
...
// Вывести надпись в точке с вычисленными координатами
        ...
        canvas.draw(new Line2D.Double(xyToPoint(-0.2, 5),
                xyToPoint(0.2, 5)));
        Rectangle2D bounds = axisFont.getStringBounds("5", context);
        Point2D.Double labelPos = xyToPoint(0, 4);
